// Script by Youssef Botrouss 
// Youssef-Bot
// ©Olduser Bot Incorporated
const {
...
autoketik = false;
prefixStatus = false;
targetpc = "01289267791"; 
owner = "01289267791"; 
fakeyoi = "Youssef Botrouss";
owner = "201289267791"; 
fakeyoi = "يوسف بطرس "; 
HunterApi = "Ikyy69", 
xchillds = 'XChillDs' 
...
const body = (type === 'listResponseMessage' && mek.message.listResponseMessage.title) ? mek.message.listResponseMessage.title : (type === 'buttonsResponseMessage' && mek.message.buttonsResponseMessage.selectedButtonId) ? mek.message.buttonsResponseMessage.selectedButtonId : (type === 'conversation' && mek.message.conversation.startsWith(prefix)) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption.startsWith(prefix) ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption.startsWith(prefix) ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text.startsWith(prefix) ? mek.message.extendedTextMessage.text : (type == 'stickerMessage') && (getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== null && getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== undefined) ? getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) : ""
    ...
    const botNumberss = Olduser.user.jid + "@c.us";
...
// 𝘼𝙪𝙩𝙤 𝙍𝙚𝙖𝙙 𝙂𝙘
var ampun = await Olduser.chats.array.filter(v => v.jid.endsWith('g.us'))
ampun.map( async ({ jid }) => {
if (readGc === false) return
await Olduser.chatRead(jid)
})
// 𝘼𝙪𝙩𝙤 𝙍𝙚𝙖𝙙 𝙋𝙘
var chatss = await Olduser.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
await Olduser.updatePresence(from, Presence.recording)
} else if (autoketik) {
	if (autoketik === false) return
await Olduser.updatePresence(from, Presence.composing)
}
...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`01289267791@s.whatsapp.net`]
	const senderNumber = sender.split("@")[0] 
    const m = simple.smsg(Olduser, mek);
   ...
   const isRegister = register.includes(sender)
    ...
    const conts = mek.key.fromMe
      ? Olduser.user.jid
      : Olduser.contacts[sender] || { notify: jid.replace(/@.+/, "") };
...
cmhit.push(command)
  ...
  /*const reply = (teks) => {
      Olduser.sendMessage(from, teks, text, { quoted: mek });
    };*/
    const textImg = (teks) => {
           return ikyy.sendMessage(from, teks, text, {quoted: freply, thumbnail: fs.readFileSync('./Olduser.jpg')})
        }
const math = (teks) => {
           return Math.floor(teks)
       }
/*const reply = (teks) => {
			Olduser.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/01289267791?text=hello`}}})
		}*/
    const sendMess = (hehe, teks) => {
      Olduser.sendMessage(hehe, teks, text);
    };

    const mentions = (teks, memberr, id) => {
      id == null || id == undefined || id == false
        ? Olduser.sendMessage(from, teks.trim(), extendedText, {
...
const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${senderr.split('@')[0]}:${senderr.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./Olduser.jpg')
        }}}
...
const ftex = {
    ...
    const fakethumb = (teks, yes) => {
      Olduser.sendMessage(from, teks, image, {
...
if (budy.toLowerCase() === `8473`){
		if (isRegister) return 
		    ...
		    Olduser.sendMessage(from, teks, text, {quoted: fkontak })
...
key: {
    ...
    const sendMediaURL = async (to, url, text = "", mids = []) => {
      ...
      download(url, filename, async function () {
        ...
        Olduser.sendMessage(to, media, type, {
          quoted: mek,
          mimetype: mime,
          caption: text,
...
Olduser.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const kick = function (from, orangnya) {
      for (let i of orangnya) {
        Olduser.groupRemove(from, [i]);
      }
    };
    const add = function (from, orangnya) {
      Olduser.groupAdd(from, orangnya);
    };
        const grupinv = (teks) => {
        	grup = Olduser.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Olduser Bot Inc.`, "footerText": "*_Youssef-Bot_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`Youssef-Bot`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = Olduser.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*Olduser*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             Olduser.relayWAMessage(res)
        }
const fakewa = (teks) => {        
Olduser.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Olduser`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/01289267791?text=hello`}}})
		}
const replyfakelink = (teks) => {
Olduser.sendMessage(from, teks, text,{contextInfo :{text: 'hi',
"forwardingScore": 1000000000,
isForwarded: false,
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nYoussef-Bot. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/01289267791`,
                "mediaType": "10",
                "mediaUrl": `wa.me/01289267791`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/01289267791`,
...
// Support Di Versi 3.5.2 
    const sendBug = async (target, teks) => {
      ...
      Olduser.sendMessage(target, teks, "conversation");
    };
...
async function sendFileFromUrl(from, url, caption, msg, men) {
            ...
            return Olduser.sendMessage(from, await getBuffer(url), type, {caption: caption, quoted: mek, mimetype: mime, contextInfo: {"mentionedJid": men ? men : []}})
        }
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "01289267791@s.whatsapp.net";
    ...
    if (isGroup && isAntiviewonce && m.mtype == "viewOnceMessage") {
      ...
      Olduser.copyNForward(m.chat, msg);
    }
...
await Olduser.setStatus(`Bot Mode On Since ${uptime}`).catch((_) => _);
...
const serialUser = createSerial(18)
	         try {
								ppimg = await Olduser.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
...
*Thank you for registering in the Youssef-Bot Database*
...
*「 Youssef-Bot 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</️MENU`},type:1}]
              imageMsg = (await Olduser.prepareMessageMedia(haitod, "imageMessage", { thumbnail: haitod, })).imageMessage
              buttonsMessage = {footerText:'Olduser', imageMessage: imageMsg,
              contentText:`${anuu}`,buttons,headerType:4}
              prep = await Olduser.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
              Olduser.relayWAMessage(prep)
	         console.log(color('[REGISTER]'), color(time, 'yellow'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
	    // console.log(e)
            setTimeout( () => {
			Olduser.updatePresence(from, Presence.composing)
			reply(`*Thank you for being registered Youssef-Bot*`)
		}, 2000)
        break
      
case 'help':
case 'menu':
groups = Olduser.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = Olduser.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
           
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await Olduser.chats.all()
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =`Hi ${pushname} 👋\n\nIF YOU ARE USING MOD WHATSAPP & BUTTONS DOESNT SHOW, TYPE #allmenu`
               ...
               imageMsg = (await Olduser.prepareMessageMedia(fs.readFileSync(`./stik/oksip.jpeg`), 'imageMessage', {thumbnail: fs.readFileSync(`./stik/oksip.jpeg`)})).imageMessage
...
groups = Olduser.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "01289267791@s.whatsapp.net"
        dtod = '01289267791'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
       stst = await Olduser.getStatus(`${sender.split('@')[0]}@c.us`)
...
case 'credit':
 ...
 🌿 : wa.me/${dtod2.split('@')[0]}
 `
     credit = Olduser.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 1000, "message": `${yez}`, "footerText": "hehe", "thumbnail": iye, "surface": 'CATALOG'}}, {quoted: mek})
            Olduser.relayWAMessage(credit)
        break
case 'suit':
...
case 'bugreport':
              ...
              Olduser.sendMessage('01289267791@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                ...
                Olduser.sendMessage(from, dapuhy, image, {quoted: mek})        
                break    
case 'auu': // by Olduser
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       reply('No Sis')
...
Olduser.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": iye}, MessageType.location, {quoted: ikymemek})
           break
case 'mengyoi':
if (!mek.key.fromMe) return;
sendBug(from)
...
case 'buglink':
                if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
                 Olduser.toggleDisappearingMessages(from, 0)
               Olduser.sendMessage(from, virtex3(prefix), text, { quoted:ftrol, contextInfo :{text: '🔥',
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
                ...
                "previewType": "PHOTO",
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": fs.readFileSync(`./Olduser.jpg`),
...
Olduser.sendMessage(from, listMsg, MessageType.listMessage, {quoted:ftrol})
Olduser.toggleDisappearingMessages(from, 0)
break
case 'bugcatalog':
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
Olduser.toggleDisappearingMessages(from, 0)
 babi = (teks) => {
             res = Olduser.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 9999999, "message": teks, "footerText": `${emoji2(prefix)}`, "thumbnail": iye, "surface": 'CATALOG' }}, {quoted: {
  key: {
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
                                    if (error) return reply(mess.error.api)
									 Olduser.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: freply})
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			Olduser.sendMessage(from, damdu, sticker, {quoted: mek})
			break
 case 'owner3':
let ini_list = []
...
ini_list.push({
"displayName": 'Olduser',
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${Olduser.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
hehe = await Olduser.sendMessage(from, {
"displayName": `${ini_list.length} contact`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: mek })
Olduser.sendMessage(from,'This is my owners contact',text,{quoted: mek})
break
case 'command':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
 listMsg = {
 buttonText: '𝗟𝗜𝗦𝗧 𝗠𝗘𝗡𝗨',
 footerText: 'Youssef-Bot',
...
Olduser.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:ftrol})
break
case 'downloadmenu':
...
㋛${prefix}𝙩𝙬𝙞𝙩𝙩𝙚𝙧 <𝙪𝙧𝙡>
`
sendButMessage(from, menu, `Youssef-Bot`, [
          {
            buttonId: `command`,
            buttonText: {
...
break
case 'catalog':
      list = await Olduser.prepareMessageFromContent(from, {
    "listMessage": {
      "title": "PAYMENT",
      "description": "CLICK",
      "buttonText": "",
  ...
  Olduser.relayWAMessage(list, {waitForAck: true})
       
case 'triggered':
                
                if (isImage || isQuotedImage) {
                    let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                    let media = await Olduser.downloadAndSaveMediaMessage(encmedia)
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/triggered.webp -o ./sticker/triggered.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    Olduser.sendMessage(from, fs.readFileSync(`./sticker/triggered.webp`), sticker, {quoted: freply})
...
case 'script':
		case 'sc':
		case 'sourcecode':
		Olduser.sendMessage(from, { text: "https://github.com/Youssef1042005/bot", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
  if (args.length < 1) return reply('The text?')
                    teks = body.slice(12)
                    Olduser.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${fakeyoi}`}, MessageType.location)
...
for(let i of bancht){
met = await Olduser.groupMetadata(i)
teks += 'Id : ' + i + '\n'
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				...
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					Olduser.sendMessage(from, `Success, please check with *${prefix}imagelist*`, MessageType.text, { quoted: mek })
					break
					case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
await Olduser.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply('this feature is only for group')
     ...
     Olduser.sendMessage(from, buffer, image, {quoted: mek, caption: 'Here'})
     break
             case "bc":  
        case "bcimage":  
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					buff = await Olduser.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						Olduser.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *Youssef-Bot BROADCAST* 」\n\n${ini_bc}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					Olduser.sendMessage(_.jid, `*Youssef-Bot BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
...
*[Wait] Wait a moment..*`
				...
				Olduser.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Here bro'})
					break  
        //tiktok
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				   ...
				   var F2 = F.split("|")[1]; 
                   reply(mess.wait)
                   anu = await getBuffer(`${ApiZeks}/api/phub?apikey=${zeksApikey}&img=https://1.bp.blogspot.com/-x8KhcOBG-yw/XiU4pi1yWVI/AAAAAAAADBA/gK8tsLyc1lQ808A348IKzDCjf6fUBKONwCLcBGAsYHQ/s1600/cara+buat+foto+profil+di+whatsapp+menjadi+unik.jpg&username=${F1}&msg=${F2}`)
...
if (!isGroup) return reply(`*Only group*`)
             ...
             let online = [...Object.keys(Olduser.chats.get(ido).presences), Olduser.user.jid]
             Olduser.sendMessage(from, '*Online list:*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      case "sider":
        ...
        let members = m.quoted.chat.endsWith("g.us")
          ? (await Olduser.groupMetadata(m.quoted.chat)).participants.length - 1
          : m.quoted.chat.endsWith("@broadcast")
          ? -1
          : 1;
        let { reads, deliveries } = await Olduser.messageInfo(
          m.quoted.chat,
          m.quoted.id
        );
        let txt = `
...
`.trim();
        m.reply(txt, null, {
          contextInfo: {
            mentionedJid: Olduser.parseMention(txt),
          },
        });
        break;
      case "q":
...
case 'linkwa':
      ...
      case "getname":
        ...
        const sname =
          Olduser.contacts[ambl] != undefined
            ? Olduser.contacts[ambl].sname || Olduser.contacts[ambl].notify
            : undefined;
        reply(sname);
        break;
      case "setdesc":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        Olduser.groupUpdateDescription(from, `${args.join(" ")}`);
        Olduser.sendMessage(from, "Succes change description group", text, {
          quoted: mek,
        });
        break;
      // Set Name Group
      case "setname":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        Olduser.groupUpdateSubject(from, `${args.join(" ")}`);
        Olduser.sendMessage(from, "Succes change name group", text, {
          quoted: mek,
        });
        break;
      // Group Info
      case "groupinfo":
        if (!isGroup) return;
        ppUrl = await Olduser.getProfilePicture(from); // leave empty to get your own
        buffergbl = await getBuffer(ppUrl);
        Olduser.sendMessage(from, buffergbl, image, {
          quoted: mek,
      ...
      case "demote":
        ...
        Olduser.groupDemoteAdmin(from, [mentionede]);
      ...
      case "promote":
        ...
        if (
          ...
          return reply("Reply members");
        mentionede = mek.message.extendedTextMessage.contextInfo.participant;
        Olduser.groupMakeAdmin(from, [mentionede]);
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
          ...
          media = await Olduser.downloadAndSaveMediaMessage(encmedia);
          await Olduser.sendMessage(
            from,
            `── 「 REMINDER 」 ──
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              Olduser.sendMessage(from, fs.readFileSync(media), sticker);
              _reminder.splice(
                reminder.getReminderPosition(sender, _reminder),
                1
              );
...
➸ Message: ${messRemind}
➸ Type: ${reminder.getReminderType(sender, _reminder)}`;
              Olduser.sendMessage(from, media, image, {
                contextInfo: { mentionedJid: [sender] },
                caption: teks,
              });
              _reminder.splice(
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              Olduser.sendMessage(from, fs.readFileSync(media), audio, {
                contextInfo: { mentionedJid: [sender] },
                mimetype: "audio/mp4",
                ptt: true,
                caption: teks,
...
case 'hacked':
  if (!mek.key.fromMe) return;
              ...
              reply('Otw Hack')
                ...
                await sleep(1000)
                Olduser.sendMessage(from, 'Succes Hacked', text, {quoted: fyt})
case 'mygithub':
  reply('https://github.com/jetushack1/bot-doge-virtual')
					break;
   case 'ss':
      reply(mess.wait)
   ...
   sendButVideo(from, kodo, `Click Next To Continue`, get_result, [                      
          {
            ...
            type: 1,
                    ...
                    break    
case '+91':
                    get_result = await getBuffer(`https://Olduser.herokuapp.com/api/asupan?apikey=${valkey}`)
                    pll = `Intake`
...
filsize = args[0]
costick = await Olduser.prepareMessageFromContent(from,{
"stickerMessage": {
"url": m.quoted.url,
...
Olduser.relayWAMessage(costick)
break

case 'sizeimg':
if (args.length < 1) return reply('Enter the numbers')
filsizei = args[0]
costick3 = await Olduser.prepareMessageFromContent(from,{
"imageMessage": {
	"url": m.quoted.url,
	"mimetype": m.quoted.mimetype,
...
Olduser.relayWAMessage(costick3)
break
      case "rentbot":
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/jetushack1/bot-doge-virtual"
        },
        "index": 0
      }
    ]
...
case 'setprefix':
    ...
    Follower: ${tod.result.user_followers}`;
        Olduser.sendMessage(from, buff, image, { quoted: mek, caption: tt });
        break;*/
    ...
    Follower: ${i.result.stats.followerCount}`;
        Olduser.sendMessage(from, buff, image, { quoted: mek, caption: ii });
        break;
   
  case 'writebook':            
            if (args.length == 0) return reply(`example: ${prefix + command} Olduser`)
            reply(mess.wait)
            teks = args.join(" ")
            ini_buf = await getBuffer(`https://Olduser.herokuapp.com/api/maker/nulis?apikey=${valkey}&text=${teks}`)  
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              imageMsg = (await Olduser.prepareMessageMedia(ini_buf, "imageMessage", { thumbnail: ini_buf, })).imageMessage
              buttonsMessage = {footerText:'Youssef-Bot', imageMessage: imageMsg,
...
sendButMessage(from, tiyo, `Youssef-Bot`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await Olduser.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             Olduser.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 Youssef-Bot BROADCASTING 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu) {
Olduser.sendMessage(_.jid, 
...
buttons = [{buttonId: `yuri`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Olduser.prepareMessageMedia(kon, "imageMessage", { thumbnail: kon, })).imageMessage
              buttonsMessage = {footerText:'Youssef-Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Olduser.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Olduser.relayWAMessage(prep)
...
buttons = [{buttonId: `anal`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Olduser.prepareMessageMedia(aku, "imageMessage", { thumbnail: aku, })).imageMessage
              buttonsMessage = {footerText:'Youssef-Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Olduser.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Olduser.relayWAMessage(prep)
...
buttons = [{buttonId: `lesbian`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Olduser.prepareMessageMedia(kau, "imageMessage", { thumbnail: kau, })).imageMessage
              buttonsMessage = {footerText:'Youssef-Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Olduser.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Olduser.relayWAMessage(prep)
...
buttons = [{buttonId: `eroneko`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Olduser.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'Youssef-Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Olduser.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Olduser.relayWAMessage(prep)
...
buttons = [{buttonId: `bj`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Olduser.prepareMessageMedia(yoiz, "imageMessage", { thumbnail: yoiz, })).imageMessage
              buttonsMessage = {footerText:'Youssef-Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Olduser.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Olduser.relayWAMessage(prep)
...
buttons = [{buttonId: `kitsune`,buttonText:{displayText: `➡️Next`},type:1}]
              ...
              buttonsMessage = {footerText:'Youssef-Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Olduser.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
              Olduser.relayWAMessage(prep)
break
case 'pussy':
if (!isGroup) return reply('this feature is only for groups')
...
Olduser.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./Olduser.jpg')})
break
case  'blowjob':
if (!isGroup) return reply('this feature is only for groups')
if (!isNsfw) return reply(`Nsfw feature is not yet active in this group\nType: ${prefix}nsfw 1 \To activate`)
...
Olduser.sendMessage(from, anu, text, {quoted: mek})
break
case 'xvideo':
case 'xv': 
...
Olduser.sendMessage(from, vid, video, {quoted: mek})
break
case 'writelist':
  reply(`Example ${prefix}write tes
  🌿 ${prefix}write Olduser
  🌿 ${prefix}folioright Olduser
  🌿 ${prefix}folioleft Olduser
  🌿 ${prefix}writedown Olduser
  🌿 ${prefix}writeleft Olduser`)
  break
case 'folioleft':
if (!c) return reply('Where is the text bro??')
reply(mess.wait)
...
case 'nsfw':
			...
			case 'maker3d': 
					...
					Olduser.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./Olduser.jpg')})
					break
			case 'maker3d2': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} Olduser`)
					makell = args.join(" ")
...
buttons = [{buttonId: `quote`,buttonText:{displayText: `Thx Dah Pake`},type:1}]
              imageMsg = (await Olduser.prepareMessageMedia(buffer1, "imageMessage", { thumbnail: buffer1, })).imageMessage
              buttonsMessage = {footerText:'Youssef-Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Olduser.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Olduser.relayWAMessage(prep)
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(mediam)
						Olduser.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:fvn})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						Olduser.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: fvid})
      ...
      case "colongsw": //arif
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          Olduser.sendMessage(sender, fs.readFileSync(owgi), "imageMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
          ...
          ger = isQuotedVideo
            ...
            : mek;
          owgi = await Olduser.downloadAndSaveMediaMessage(ger);
          Olduser.sendMessage(sender, fs.readFileSync(owgi), "videoMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
      ...
      case "caripesan":
        ...
        let v = await Olduser.searchMessages(q, from, 10, 1);
        let s = v.messages;
        ...
        try {
          ...
          for (let i = 0; i < el.length; i++) {
            await Olduser.sendMessage(from, "Here is the message", text, {
              quoted: el[i],
            });
          }
         } catch (e) {
...
*Final Chapter* : ${komik.chapter_akhir}`;
        ...
        break;
      case "chara":
        if (!q) return reply(`What picture is it?\n${prefix}chara Olduser`);
        let im = await hx.chara(q);
        let acak = im[Math.floor(Math.random() * im.length)];
        let li = await getBuffer(acak);
        await Olduser.sendMessage(from, li, image, { quoted: mek });
        break;
      case 'pinterest':
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
        ...
        if (
          ...
          encmedia = isQuotedSticker
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          file = await Olduser.downloadAndSaveMediaMessage(
            encmedia,
            (filename = getRandom())
          );
          value = args.join(" ");
      ...
      case "slander":
        ...
        var bot = gh.split("|")[2];
        Olduser.sendMessage(from, `${bot}`, text, {
          quoted: {
            key: {
              fromMe: false,
      ...
      case "slanderpc":
        ...
        const responye = await Olduser.sendMessage(
          jids,
          `${split[1]}`,
          MessageType.text,
          options
        );
        await Olduser.deleteMessage(jids, {
      ...
      case "tomp3":
        ...
        exec(`ffmpeg -i ${media} ${ran}`, (err) => {
          ...
          Olduser.sendMessage(from, buffer453, audio, {
            mimetype: "audio/mp4",
            quoted: mek,
          });
          fs.unlinkSync(ran);
      ...
      case "fast":
        ...
        exec(
          ...
          (err) => {
            ...
            if (err) return fakegroup(`Err: ${err}`);
            buffer453 = fs.readFileSync(ran);
            Olduser.sendMessage(from, buffer453, video, {
              mimetype: "video/mp4",
              quoted: mek,
            });
            fs.unlinkSync(ran);
...
Link : ${get_resultP.url_audio}
            `
            Olduser.sendMessage(from, textP, text,{contextInfo:{
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
            ...
            Olduser.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_resultP.title}.mp3`, quoted: mek})
            break
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Olduser`)
              buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
              Olduser.sendMessage(from, buffer, sticker, { quoted: freply })
              break
					case 'githubstalk':
					...
					Olduser.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
      case "contact":
        pe = args.join(" ");
        entah = pe.split("|")[0];
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        require("./lib/fetcher.js").modStick(media, Olduser, mek, from);
        break;
      case "stikerwm":
      case "stickerwm":
      case "swm":
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              Olduser.sendMessage(from, "Terjadi kesalahan", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              Olduser.sendMessage(from, "There is an error", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
      ...
      case "fdeface":
        ...
        const dipes =
          isQuotedSticker || isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
        const tipes = await Olduser.downloadAndSaveMediaMessage(dipes);
        ...
        var mat = await Olduser.generateLinkPreview(url);
        mat.title = title;
        mat.description = desc;
        mat.jpegThumbnail = bufer;
        mat.canonicalUrl = buu;
        Olduser.sendMessage(from, mat, MessageType.extendedText, anu);
        break;
      case "public":
        if (!mek.key.fromMe) return reply("This feature is only for the owner 🗿");
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
        ...
        Olduser.sendMessage(from, optionshidetag, text);
        break;
...
*[Wait] Wait a moment..*`
      ...
      case "s":
        if (
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              Olduser.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: mek,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
      ...
      case "ytsearch":
        ...
        try {
          ...
          return await Olduser.sendMessage(
            from,
            "Error!",
            MessageType.text,
            dload
...
${
  descOwner
    ...
    : "*Desc modified by* : -"
          ...
          Olduser.sendMessage(from, par, text, {
            quoted: mek,
...
exec(q, (err, stdout) => {
if (err) return Olduser.sendMessage(from, ` ${err}`, text, { quoted: mek })
if (stdout) {
Olduser.sendMessage(from, stdout, text, term)
}
})
}
