...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`994408359440@s.whatsapp.net`]
...
/*const reply = (teks) => {
			Olduser.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/994408359440?text=hello`}}})
		}*/
...
Olduser.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      Olduser.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = Olduser.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Olduser Bot Inc.`, "footerText": "*_𝐃𝐨𝐠𝐞 𝐁𝐨𝐭_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakewa = (teks) => {        
Olduser.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Olduser`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/994408359440?text=hello`}}})
		}
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/994408359440`,
                "mediaType": "10",
                "mediaUrl": `wa.me/994408359440`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/994408359440`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "994408359440@s.whatsapp.net";
...
groups = Olduser.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "994408359440@s.whatsapp.net"
        dtod = '994408359440'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
case 'bugreport':
              ...
              Olduser.sendMessage('994408359440@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
     ...
     break
        ...
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					buff = await Olduser.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						Olduser.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *𝐃𝐨𝐠𝐞 𝐁𝐨𝐭 BROADCAST* 」\n\n${ini_bc}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					Olduser.sendMessage(_.jid, `*𝐃𝐨𝐠𝐞 𝐁𝐨𝐭 BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             Olduser.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 𝐃𝐨𝐠𝐞 𝐁𝐨𝐭 BROADCASTING 」*\n\n${body.slice(4)}`})
}
